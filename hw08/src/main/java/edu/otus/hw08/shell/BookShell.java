...
import edu.otus.hw08.entities.Comment;
...
public class BookShell {
    ...
    public void add_comment(@ShellOption String title,
                            @ShellOption String comment) {
        try {
            Book book = bookService.getByTitle("title");
            book.getComments().add(Comment.builder().commentText(comment).build());
            bookService.add(book);
        } catch (NoSuchElementException e) {
            System.out.println("No book with title " + title + " found");
        }
    }
